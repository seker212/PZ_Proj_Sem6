...
namespace DatabaseAPI.DAL
{
    public class ProductRepository : ObjectRepository<Product>, IProductRepository
    {
        ...
        public IEnumerable<(Guid id, string name, int quantity)> GetDiscountProducts(Discount discount) => Query().Select("products.id", "products.name", "discounts_set_items.quantity").Join("discounts_set_items", "product_id", "id").Where("product_id", discount.Id).Get<(Guid id, string name, int quantity)>();
