...
namespace DatabaseAPI.DAL
{
    public class PairRepository<T> : Repository<T>, IPairRepository<T> where T : IDbModel
    {
        ...
        public T Get(IEnumerable<Guid> primaryKeys) => primaryKeys.Count() == 2 ? Query().Where(_columnNames[0], primaryKeys.ElementAt(0)).Where(_columnNames[1], primaryKeys.ElementAt(1)).First<T>() : throw new ArgumentException(); //TODO: Use more descriptive exception

        public override T Get(object primaryKey)
        {
            var primaryKeys = primaryKey as IEnumerable<Guid> ?? throw new ArgumentNullException(); //TODO: Use more descriptive exception
