...
namespace DatabaseAPI.Controllers
{
    ...
    public class CrudOrderItemsController : ControllerBase
    {
        ...
        public async Task<IActionResult> Post([FromHeader] string sessionId, [FromBody] DatabaseModels.OrderItems orderItems)
        {
            if (Startup.ActiveSessions.ContainsKey(sessionId) && Startup.ActiveSessions[sessionId].Type == DatabaseModels.UserType.Admin)
            {
                var result = await _services.Create(orderItems);
                return Ok(result);
            }
            else
            {
        ...
        [ProducesResponseType(StatusCodes.Status200OK)]
        [ProducesResponseType(StatusCodes.Status401Unauthorized)]
        public async Task<IActionResult> Update([FromHeader] string sessionId, [FromBody] DatabaseModels.OrderItems orderItems)
        {
            if (Startup.ActiveSessions.ContainsKey(sessionId) && Startup.ActiveSessions[sessionId].Type == DatabaseModels.UserType.Admin)
            {
                var result = await _services.Update(orderItems);
                return Ok(result);
            }
            else
            {
        ...
        [ProducesResponseType(StatusCodes.Status401Unauthorized)]
        [ProducesResponseType(StatusCodes.Status404NotFound)]
        public async Task<IActionResult> Delete([FromHeader] string sessionId, [FromBody] DatabaseModels.OrderItems orderItems)
        {
            if (Startup.ActiveSessions.ContainsKey(sessionId) && Startup.ActiveSessions[sessionId].Type == DatabaseModels.UserType.Admin)
            {
